...
var GUID_SCRIPT = '/opt/smartdc/imgapi/tools/get-image-dataset-guid.sh';
...
function getDatasetGuid(image, files, callback) {
    var guid;
    var execArgs = [image.uuid, files[0].compression || 'none'];
 ...
 */
function bigDecimalFromHex(hex) {
    function add(x, y) {
        var c = 0, r = [];
        ...
        if (config.database.type === 'ufds') {
            ...
            fs.writeFile(dbPath, content, 'utf8', function (err) {
                ...
                getNobody(function (nobody) {
                    ...
                    fs.chown(dbPath, nobody.userId, nobody.groupId, callback);
                });
            });
        }
    });
