...
const DEFAULT_REGISTRIES = {
    images: 'https://us.lxd.images.canonical.com',
...
function getImageBestFile(img, version) {
    ...
    if (items.hasOwnProperty('lxd_combined.tar.gz')) {
        ...
        // The "best" file depends on system capabilities and the files included
        // in the image manfiest. This determines the fingerprint reported by
        // lxc.
        // You can literally be running the exact same bits on two different
        // systems, but the fingerprint will be reported differently if you
        // don't support squashfs. In all cases, we want to use the same
        // fingerprint that lxc by itself would on our Linux platform images.
        // Note that these names are arbitrary, and by convention. What we're
        // really looking for is the ftype. Most often, the item key is the
        // same as the ftype (e.g., root.tar.xz and root.tar.xz) but that's not
        // the case for the images: server, which uses root.squashfs instead of
        // just squashfs. There *should* only be one file of each ftype in
        // items but that is not guaranteed to be the case.
        // Discussing with the LXC folks, multiple files of the same ftype would
        // likely result in indeterministic behavior.
        var itemFromFtype = {};

        Object.keys(items).forEach(function (name) {
            itemFromFtype[items[name].ftype] = items[name];
        });

        /* BEGIN JSSTYLED */
        // jsstyled this to make it easier to read and maintain in the future.
        bestFile = itemFromFtype['squashfs']
                || itemFromFtype['root.tar.xz']
                || itemFromFtype['disk-kvm.img']
                || itemFromFtype['uefi1.img']
                || itemFromFtype['disk1.img']
            ;
        /* END JSSTYLED */
...
if (require.main === module) {
    ...
    var lxdOpts = {
        log: log,
        url: 'https://us.lxd.images.canonical.com'
        // url: 'https://cloud-images.ubuntu.com/releases'
    };
    var wantedImage = 'alpine/3.16';
