    ...
    function exportImage() {
        async.waterfall([
            function exportManifest(next) {
                ...
                var filename = format('%s-%s.%s',
                    image.name, image.version, 'imgmanifest');
                // response.manifest_path
                manifestStorPath = path.resolve(mpath, filename);

                stor.exportImageManifest(
                stream, string, manifestStorPath, mpath, function (sErr) {
                    if (sErr) {
            ...
            function exportImageFile(next) {
                ...
                fstor.snapLinkImageFile(image, fileStorPath, function (sErr) {
                    if (sErr) {
