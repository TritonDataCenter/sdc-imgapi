...
function apiCreateImage(req, res, next) {
    ...
    if (req.query.user) {
        ...
        if (!UUID_RE.test(user)) {
            return next(new errors.InvalidParameterError(
                format('invalid user: not a UUID: "%s"', user),
                [{field: 'user', code: 'Invalid'}]));
        }
...
function reqGetImage(req, res, next) {
    ...
    var uuid = req.params.uuid;
    log.debug({uuid: uuid, user: user}, 'get image');
    Image.get(req._app, uuid, log, function (getErr, image) {
        ...
        if (user) {
            // When `?user=$uuid` is used we restrict to images accessible
            // to this user -> 404 if no access.
            var access;
            if (image.owner === user) {
                // User's own image.
                access = true;
                ...
                log.debug({image: image, user: user},
                    'access denied: inactive image owned by someone else');
                access = false;
            } else if (image.public) {
                // Public active image.
                access = true;
            } else if (image.users && image.users.indexOf(user) !== -1) {
                // Private active image of which `user` is a designated user.
                access = true;
            } else {
                log.debug({image: image, user: user},
                    'access denied: private image, not a designated user');
                access = false;
            }
            if (!access) {
                return next(new errors.ResourceNotFoundError(
