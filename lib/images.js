...
var MAX_ICON_FILE_SIZE = 128*1024; // 128KiB
    ...
    if (reqs === undefined) {
        ...
        if (reqs.min_platform) {
            if (typeof (reqs.min_platform) !== 'object' ||
                ...
                for (var mKey in reqs.min_platform) {
                    var imgVer = reqs.min_platform[mKey];
                    if (validPlatformVersion(imgVer) === false) {
                        errs.push({
                            ...
                            + '"requirements.min_platform" entry: "%s: %s" '
                                + 'not a valid platform version', mKey, imgVer)
                        });
                    }
                }
            }
        ...
        if (reqs.max_platform) {
            if (typeof (reqs.max_platform) !== 'object' ||
                ...
                for (var mKey in reqs.max_platform) {
                    var imgVer = reqs.max_platform[mKey];
                    if (validPlatformVersion(imgVer) === false) {
                        errs.push({
                            field: 'requirements.max_platform',
                            code: 'Invalid',
...
function apiAdminImportImage(req, res, next) {
    ...
    Image.get(app, data.uuid, log, function (gErr, image) {
        ...
        Image.create(app, data, true, function (cErr, newImage) {
            if (cErr) {
                return next(cErr);
            }
...
function apiAddImageIcon(req, res, next) {
    ...
    if (contentLength !== undefined && contentLength > MAX_ICON_FILE_SIZE) {
        finish(new errors.UploadError(format(
            'icon size %s (from Content-Length) exceeds the maximum allowed '
            + 'size, 128KiB', req.headers['content-length'])));
    }
    ...
    req.on('data', function (chunk) {
        ...
        if (size > MAX_ICON_FILE_SIZE) {
            finish(new errors.UploadError(format(
                'icon size exceeds the maximum allowed size, 128KiB')));
        }
    });
    req.on('end', function () {
        req.log.trace('req "end" event');
    ...
    stor.storeFileFromStream(req._image, req, 'icon', function (sErr) {
        if (sErr) {
            ...
            finish(errors.parseErrorFromStorage(
                err, 'error receiving image icon'));
        } else {
            finish();
        }
    });
