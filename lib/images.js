...
function Image(app, raw) {
    ...
    this.destroyed_at = this.raw.destroyed_at &&
        new Date(this.raw.destroyed_at);
...
Image._calcState = function (activated, disabled) {
    var destroyed = this.destroyed_at;
...
Image.prototype.serialize = function serialize(mode) {
    ...
    if (this.destroyed_at) data.destroyed_at = this.raw.destroyed_at;
 ...
 *      use for an API reponse (i.e. doesn't expose internal details).
 */
Image.prototype.markAsDestroyed = function markAsDestroyed(app, log, callback) {
    var modification = {};
    this.state = this.raw.state = 'destroyed';
    this.destroyed_at = new Date();
    this.raw.destroyed_at = this.destroyed_at.toISOString();

    modification.destroyed_at = this.raw.destroyed_at;
    modification.state = this.raw.state;
    var change = {
        operation: 'replace',
        modification: modification
...
function apiListImages(req, res, next) {
    ...
    } else if (req.query.state === 'creating' ||
        req.query.state === 'failed' || req.query.state === 'destroyed') {
        if (limitToActive) {
            res.send([]);
            return next();
        }
        ...
        function addItToDb(image, next) {
            if (placeholder) {
                    ...
                    !imgadmVersionFromReq(req))
                {
                    ...
                    image.raw.requirements = JSON.stringify(image.requirements);
...
function apiDeleteImage(req, res, callback) {
    ...
    // Can only purge image if owner was not passed and purge=true
    function canPurgeImage() {
        return req.query.account === undefined && (req.query.purge &&
                (req.query.purge === 'true' || req.query.purge === true));
    }
    ...
    async.series([
        ...
        function deleteModel(next) {
            // Only keep model if app mode is dc and purge was not passed
            if (app.mode === 'dc' && !canPurgeImage()) {
                image.markAsDestroyed(app, log, function (err) {
                    if (err) {
                        return next(err);
                    }
                    next();
                ...
                // Delete the model.
                // Note: We delete the manifest entry first to make sure the
                // entry goes away, if subsequent deletion of files from storage
                // fails, then that is just internally logged for operators to
                // cleanup.
        ...
        function deleteFiles(next) {
            // Idempotent: if image files are already destroyed (image is
            // already marked as destroyed, skip this step)
            if (image.state === 'destroyed') {
                return next();
            }
