                    ...
                    if (err) {
                        ...
                        return next(new errors.RemoteSourceError(err,
                            format('Unable to get manifest for origin image %s. ' +
                                'Error from remote: %s', currentOrigin,
                                err.message || err.code)));
                    }
 ...
 */
Image.filter = function filterImages(app, options, log, limitOpts, callback) {
    ...
    // Do the search.
    app.db.search(options, limitOpts, function (err, rawItems) {
        if (err) {
            if (err.statusCode === 503) {
                return callback(err);  // don't cache 503
            } else {
    ...
    function filterImages(_id) {
        ...
        async.forEach(filterOpts,
            function filterOne(opts, nextAsync) {
                Image.filter(app, opts, log, limOpts, function (cErr, images) {
                    if (cErr) {
...
function apiUpdateImage(req, res, next) {
    ...
    Image.modify(req._app, image, req.log, function (err) {
        if (err) {
            return next(err);
        }
        res.send(image.serialize(req._app.mode));
