 ...
 */
Image.importRemote =
function importRemote(app, uuid, source, skipOwnerCheck, log, callback) {
    assert.object(app, 'app');
    assert.string(uuid, 'uuid');
    ...
    async.waterfall([
        ...
        function addImageFile (next) {
            ...
            // Hacky, read the file to get its contentMD5?
            stream.on('end', function () {
                ...
                stor.storeFileFromFile(newImage, stream, 'file0',
                    ...
                    if (sErr) {
                        ...
                        return next(errors.parseErrorFromStorage(
...
function apiAdminImportRemoteImage(req, res, next) {
    ...
    Image.get(app, uuid, log, function (gErr, image) {
        ...
        Image.importRemote(app, uuid, req.query.source, skipOwnerCheck, log,
            function (err, newImg) {
            if (err) {
                return next(err);
            }
