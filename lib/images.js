...
Image.filter = function filterImages(app, options, log, callback) {
    ...
    // Do the search.
    log.debug({ldapFilter: String(ldapFilter), options: util.inspect(options)},
        'Image.filter ufds search');
    app.db.search(ldapFilter, function (err, rawItems) {
        if (err) {
            if (err.httpCode === 503) {
...
function apiCreateImage(req, res, next) {
    ...
    Image.create(app, data, false, function (cErr, image) {
        ...
        app.db.add(image.dn, image.raw, function (addErr) {
            if (addErr) {
                log.error({dn: image.dn}, 'error saving to UFDS: raw data:',
                    image.raw);
                return next(addErr);
            }
...
function apiAdminImportImage(req, res, next) {
    ...
    Image.get(app, data.uuid, log, function (gErr, image) {
        ...
        Image.create(app, data, true, function (cErr, image) {
            ...
            app.db.add(image.dn, image.raw, function (addErr) {
                if (addErr) {
                    log.error({dn: image.dn, raw: image.raw},
                        'error saving to UFDS');
                    return next(addErr);
                }
                app.cacheInvalidateWrite('Image', image);
                res.send(image.serialize());
