...
Image.create = function createImage(app, data, isImport, callback) {
    ...
    delete data.files;
    //XXX drop a few for now: TODO add these back
    delete data.requirements; //XXX
    delete data.users; //XXX
    delete data.generate_passwords; //XXX
...
function apiCreateImage(req, res, next) {
    ...
    Image.create(app, data, false, function (cErr, image) {
        ...
        app.ufdsAdd(image.dn, image.raw, function (addErr) {
            if (addErr) {
                log.error({dn: dn, raw: image.raw},
                    'error saving to UFDS');
                return next(new errors.InternalError(addErr,
                    'internal error saving image'));
            }
