...
function apiCreateImage(req, res, next) {
    ...
    Image.create(data, false, function (cErr, image) {
        ...
        if (mode === 'public' && !image.public) {
            return next(new errors.NoPrivateImagesError());
        }
...
function apiUpdateImage(req, res, next) {
    ...
    // Merge new values into existing raw image and then validate it
    ...
    // Revalidate the object so we run through the same code path as creation
    // We also validate if the image is in a public server and a client is
    // trying to make it private
    try {
        var image = new Image(raw);
        if (req._app.mode === 'public' && !image.public) {
            throw new errors.NoPrivateImagesError();
        }
    } catch (cErr) {
        return next(cErr);
    }
