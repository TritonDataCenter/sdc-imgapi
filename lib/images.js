...
Image.validate = function validateImage(app, raw) {
    ...
    // icon
    if (raw.icon) {
        var icon = url.parse(raw.icon);
        if (icon.protocol === undefined || (icon.protocol !== 'http:' &&
            icon.protocol !== 'https:')) {
            errs.push({
                field: 'icon',
                code: 'Invalid',
                message: 'invalid image icon URL protocol'
            });
        } else if (raw.icon.length > MAX_URL_LENGTH) {
            errs.push({
                field: 'icon',
                code: 'Invalid',
                message: format('image icon URL is too long ' +
                    '(max %d characters)', MAX_URL_LENGTH)
            });
        }
    }
