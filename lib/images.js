...
Image.validate = function validateImage(app, raw) {
    ...
    if (raw.eula) {
        ...
        } else if (raw.eula.length > MAX_URL_LENGTH) {
            errs.push({
                field: 'eula',
                code: 'Invalid',
    ...
    // origin
    if (raw.origin && ! UUID_RE.test(raw.origin)) {
...
function apiCreateImageFromVm(req, res, next) {
    ...
    if (uuid === undefined) {
        manifest.uuid = uuid = genUuid();
    }

    wfapi.createImageFromVmJob(req, vmUuid, manifest, function (err, jobUuid) {
        if (err) {
            return next(err);
        }
