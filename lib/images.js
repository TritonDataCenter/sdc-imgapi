...
function apiCreateImageFromVm(req, res, callback) {
    ...
    if (!req._app.getStor('manta') &&
        !req._app.config.allowLocalCreateImageFromVm)
    {
    ...
    // that supports incremental images.
    if (incremental && !(manifest.requirements &&
                         manifest.requirements.min_platform)) {
        if (!manifest.requirements)
            manifest.requirements = {};
        manifest.requirements.min_platform = {"7.0": "20130729T063445Z"};
    }
...
function apiExportImage(req, res, callback) {
    ...
    var image = req._image;
    var stor = app.storFromImage(image, 'manta');
    if (stor.type !== 'manta') {
    ...
    if (account) {
        app.ufdsClient.getUser(account, function (err, user) {
            if (err) {
                // More like AccountDoesNotExistError?
                return callback(new errors.OwnerDoesNotExistError(
                    err, account));
    ...
    function exportImage() {
        async.waterfall([
            function exportManifest(next) {
                var string = JSON.stringify(image.serialize(app.mode));
                var stream = new MemoryStream();

                stor.exportManifestFromStream(
                image, stream, string, mpath, function (sErr) {
                    if (sErr) {
            ...
            function exportImageFile(next) {
                ...
                fstor.snapLinkImageFile(image, file, mpath, function (sErr) {
                    if (sErr) {
            ...
            res.send(image.serialize(app.mode));
            return callback(false);
        });
    }
}
