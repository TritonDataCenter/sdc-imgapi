...
Image.create = function createImage(app, data, isImport, skipOwnerCheck,
    ...
    if (isImport) {
        ...
        if (data.published_at && typeof(data.published_at) !== 'string') {
            return callback(new errors.InvalidParameterError('invalid "published_at"',
...
function apiCreateImageFromVm(req, res, next) {
    if (req.query.action !== 'create-from-vm')
        return next();
    ...
    if (uuid === undefined) {
        manifiest.uuid = uuid = genUuid();
    }

    wfapi.createImageFromVmJob(vmUuid, manifest, function (err, jobUuid) {
        if (err) {
            return next(err);
        }
        // Allow clients to know where is wfapi located
        res.header('workflow-api', req._app.config.wfapi.url);
...
// TODO
// - no content-md5 when serving a range
// - manta support
// - returns 216
// - return content-range header
function verifyRange(req, res, next) {
    ...
    if (range.indexOf(',') !== -1) {
        message = 'multi-range requests not supported';
        res.header('Content-Type', 'application/json');
        return next(new errors.NotImplementedError(message));
    }

    // Ignore for manta
    var stor = req._image.files[0].stor;
    if (stor === 'manta')
        return next();

    var size = req._image.files[0].size;
    var bytes = size - 1;

    range = range.split('=');
    if (range.length === 2 && range[0] === 'bytes') {
        var spec = range[1].split('-');
        if (spec.length === 2) {
            // suffix-byte-range-spec: bytes=-500. last 500 bytes
            if (spec[0] === '') {
                req.range = {
                    start: (bytes - Number(spec[1]) + 1),
                    end: bytes
                };
            // suffix-byte-range-spec: bytes=9500-. from 9500 to end
            } else if (spec[1] === '') {
                req.range = { start: Number(spec[0]), end: bytes };

            // byte-range-spec: bytes=0-499
            } else if (Number(spec[1]) >= Number(spec[0])) {
                var end = (Number(spec[1]) > bytes ? bytes : Number(spec[1]));
                req.range = { start: Number(spec[0]), end: end };
            }
        }
    }

    if (req.range) {
        res.header('Content-Length', (req.range.end - req.range.start + 1));
    // No parsing conditions were matched, range was invalid
    } else {
        res.header('Content-Range', 'bytes */' + size.toString());
        message = 'requested range is not satisfiable';
        res.header('Content-Type', 'application/json');
        return next(new errors.RangeNotSatisfiableError(message));
    }
...
function apiGetImageFile(req, res, next) {
    ...
    var range = req.range || {};
    stor.createImageFileReadStream(req._image, range, function (sErr, stream) {
        // TODO: handle 404?
        if (sErr) {
            return finish(sErr);
        }
