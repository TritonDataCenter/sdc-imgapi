...
Image.create = function createImage(app, data, callback) {
    ...
    var raw = {
        ...
        name: data.name,
        type: data.type,
        //XXX disable this. published_at should be set at activation
...
function apiAddImageFile(req, res, next) {
    ...
    //var outPath = "/Users/trentm/joy/imgapi/tmp/outfile"; //XXX
    var outPath = "/var/tmp/imgapi/outfile"; //XXX
    req.pipe(fs.createWriteStream(outPath));
...
function mountApi(server) {
    ...
    server.put(
        ...
        apiAddImageFile);
    server.post(
        {path: '/images/:uuid/activate', name: 'ActivateImage'},
        reqGetImage,    // add `req._image`
        apiActivateImage);
    server.del(
