...
Image.prototype.activate = function activate(app, log, callback) {
    ...
    this.raw.state = 'active';

    modification.activated = this.raw.activated;
    modification.state = 'active';
    var change = {
        operation: 'replace',
 ...
 */
Image.importImage =
function importImage(app, uuid, source, skipOwnerCheck, log, callback) {
    assert.object(app, 'app');
    assert.string(uuid, 'uuid');
    ...
    var image = {};
    image.file = path.join(TMPDIR, format('%s.zfs', uuid));

    async.waterfall([
        function downloadManifest(next) {
            client.getImage(uuid, function (err, manifest) {
                if (err) {
                    log.error(err, 'failed to download manifest for image %s',
                        uuid);
                    return next(err);
                }
                image.manifest = manifest;
                image.files = manifest.files;
                return next();
            });
        },
        function downloadImageFile(next) {
            client.getImageFile(uuid, image.file, function (err) {
                if (err) {
                    log.error(err, 'failed to download file for image %s',
                        uuid);
                    return next(err);
                }
                return next();
        ...
        // TODO: Add a sha1 check that the downloaded file was properly
        // downloaded.

        function createTheImage(next) {
            Image.create(app, image.manifest, true, skipOwnerCheck,
        ...
        function addImageFile(next) {
            var md5sum = crypto.createHash('md5');
            var newImage = image.newImage;
            var stor = app.storFromImage(newImage);
            var stream = fs.createReadStream(image.file);
            ...
            stream.on('end', function () {
                log.trace('stream "end" event');
            });
            stream.on('close', function () {
                log.trace('stream "close" event');
            });
            ...
            function finish_(err) {
                ...
                file.stor = stor.type;
                file.contentMD5 = md5sum.digest('base64');
                file.mtime = (new Date()).toISOString();

                newImage.addFile(app, file, log, function (err2) {
                    if (err2) {
                        log.error(err2, 'error adding file info to Image');
            ...
            stor.storeFileFromFile(newImage, stream, 'file0',
                function (sErr) {
                if (sErr) {
                    log.error(sErr, 'error storing image file');
        ...
        function activateImage(next) {
            image.newImage.activate(app, log, function (err) {
                if (err) {
                    return next(err);
                }
                return next();
        ...
        var newImage = image.newImage;
        // If there was an error *else* the image wasn't created at all
        if (!err) {
        ...
        // XXX If there's an error, don't remove files at TMP_DIR. That
        // way, a developer can inspect the file to see what's wrong
        // (truncated, corrupt, etc.).
        app.db.del(newImage.uuid, function (delErr) {
            if (delErr) {
                return callback(delErr);
            }
            app.cacheInvalidateDelete('Image', newImage);
            ...
            async.forEach(
                ...
                function deleteOneFile(file, nextFile) {
                    ...
                    stor.deleteImageFile(newImage, nextFile);
                },
                function doneDeletes(fileErr) {
                    if (fileErr) {
                        log.error({err: fileErr, image: newImage},
                        'error deleting model file(s), this image may have ' +
                        'zombie files which must be remove manually by an ' +
                        'operator');
                        return callback(errors.parseErrorFromStorage(fileErr,
                            'error deleting image file'));
                    }
                    // Otherwise if deleteImage went fine just return the err
                    return callback(err);
                }
            );
        });
...
Image.validate = function validateImage(app, raw) {
    ...
    try {
        ...
        errs.push({
            ...
            code: 'Invalid',
            message: format(
                'invalid image "tags" (not parseable JSON): %s',
                raw.tags)
        });
    }
    if (tags === undefined) {
...
Image.create = function createImage(app, data, isImport, skipOwnerCheck,
    ...
    if (!skipOwnerCheck && app.mode === 'dc') {
        ...
        var ufdsClient = app.ufdsClient;
        if (!ufdsClient) {
            return callback(new errors.ServiceUnavailableError(
                'service unavailable (ufds)'));
        }
        ufdsClient.getUser(raw.owner, function (err, user) {
            if (err) {
...
function apiImportImage(req, res, next) {
    if (req.query.action !== 'import')
        return next();

    // If this is just a local import then skip remoteImport
    ...
    Image.get(app, uuid, log, function (gErr, image) {
        ...
        log.debug({uuid: uuid, source: req.query.source},
            'AdminImportRemoteImage: start import');

        Image.importImage(app, uuid, req.query.source, skipOwnerCheck, log,
            function (err, newImg) {
            if (err) {
                return next(err);
            }
            res.send(newImg.serialize(app.mode));
            next(false);
        });
    });
}
...
function apiAddImageFile(req, res, next) {
    ...
    function finish_(err) {
        ...
        req._image.addFile(req._app, file, req.log, function (err2) {
            if (err2) {
...
function apiAddImageIcon(req, res, next) {
    ...
    function finish_(err) {
        ...
        req._image.addIcon(req._app, icon, req.log, function (err2) {
            if (err2) {
...
function mountApi(server, reqAuth, reqPassiveAuth) {
    ...
    server.post(
        ...
        resume,
        restify.bodyParser({mapParams: false}),
        apiImportImage,      // before `reqGetImage` b/c shouldn't be one
