...
Image.validate = function validateImage(app, raw) {
    ...
    if (raw.tag) {
        ...
        manifest.tags = utils.keyValueToObject(raw.tag);
    }

    var fn;
    if (manifest.state === 'creating') {
        fn = imgmanifest.validateMinimalManifest;
    } else if (app.mode === 'public') {
 ...
 * @param skipOwnerCheck {Boolean} If true, the check that the owner UUID
 *      exists in UFDS will be skipped.
 * @param placeholder {Boolean} If true, the image data is treated and validated
 *      as a minimal image and most of its fields can be omitted. All the image
 *      attributes will be present only after the image has been physically
 *      created (i.e. create-from-vm).
...
Image.create = function createImage(app, data, isImport, skipOwnerCheck,
    ...
    assert.bool(skipOwnerCheck, 'skipOwnerCheck');
    // placeholder is an optional argument
...
function apiCreateImageFromVm(req, res, callback) {
    ...
    async.waterfall([
        function createIt(next) {
            ...
            req.log.info({ data: data }, 'CreateImage: create it');
            Image.create(req._app, data, false, false, true, next);
        },
        function addItToDb(image, next) {
            app.db.add(image.uuid, image.raw, function (addErr) {
                if (addErr) {
...
function apiAdminImportImage(req, res, callback) {
    ...
    async.waterfall([
        ...
        function createIt(next) {
            log.info({data: data}, 'CreateImage: create it');
            Image.create(app, data, true, skipOwnerCheck, next);
        },
        function addItToDb(image, next) {
            app.db.add(image.uuid, image.raw, function (addErr) {
                if (addErr) {
        ...
        if (err.restCode === 'ImageUuidAlreadyExists' && req._placeholder) {
...
function apiUpdateImage(req, res, next) {
    ...
    // Cleanup these fields so a placeholder image can be updated
    var PLACEHOLDER_ATTRS = ['v', 'uuid', 'name', 'version', 'owner', 'files'];
    ...
    var data = req.body;
    // Cleanup these fields so a placeholder image can be updated
    if (req._image.state === 'creating') {
        for (i = 0; i < PLACEHOLDER_ATTRS.length; i++) {
            delete data[PLACEHOLDER_ATTRS[i]];
        }
    }
