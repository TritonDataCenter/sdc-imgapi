...
Image.validate = function validateImage(app, raw) {
    ...
    if (reqs === undefined) {
        ...
        } else if (VALID_BRANDS.indexOf(reqs.brand) === -1) {
            errs.push({
                field: 'requirements.brand',
                code: 'Invalid',
                message: format(
                    'invalid image "requirements.brand": invalid brand "%s" ' +
                    '(must be one of %s)', reqs.brand,
                    VALID_BRANDS.join(', '))
            });
        }
        delete reqs.brand;
...
Image.create = function createImage(app, data, isImport, callback) {
    ...
    try {
        var data = imgmanifest.upgradeManifest(data);
    } catch (err) {
