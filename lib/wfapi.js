...
Wfapi.prototype.createImageFromSnapshotJob = function (image, snapshot, cb) {
    ...
    params.task = 'image-from-snapshot';
    params.target =  format('/image-%s-from-snapshot-%s', image, snapshot);

    self.client.createJob('image-from-snapshot', params, function (err, job) {
        if (err) {
            cb(err);
            return;
        }

        self.log.debug('Image from snapshot job ' + job.uuid +
            ' queued for Image ' + image);
        cb(null, job.uuid);
    });
};
