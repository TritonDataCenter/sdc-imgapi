 ...
 */
Wfapi.prototype.createImageFromVmJob = function (req, vmUuid, manifest, cb) {
    var self = this;
    // Default to gzip for now
    ...
    var params = {
        ...
        manifest: manifest,
        task: 'create-from-vm',
        target: format('/create-from-vm-%s', vmUuid)
    };
    var options = { headers: getRequestHeaders(req) };

    self.client.createJob(params.task, params, options, function (err, job) {
        if (err) {
            return cb(err);
        }
 ...
 */
Wfapi.prototype.createImportRemoteImageJob =
function (req, uuid, source, manifest, skipOwnerCheck, cb) {
    var self = this;
    var params = {
        image_uuid: uuid,
        source: source,
        ...
        target: format('/import-remote-%s', uuid)
    };
    var options = { headers: getRequestHeaders(req) };
    ...
    self.client.createJob(params.task, params, options, function (err, job) {
        ...
        params.job_uuid = job.uuid;
        self.log.debug(params, 'Import remote image job params');
        return cb(null, job.uuid);
    });
};
