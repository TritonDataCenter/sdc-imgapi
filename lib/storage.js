...
function LocalStorage(options) {
    ...
    assert.string(config.archiveDir, 'options.config.archiveDir');
    ...
    this.archiveDir = path.resolve(config.archiveDir);
...
function MantaStorage(options) {
    ...
    this.archiveDir = format('/%s/stor/imgapi-archive/%s', this.user,
...
function (stream, string, storPath, callback) {
    var self = this;
    var opts = {
...
function (stream, manifest, callback) {
    assert.object(manifest, 'manifest');
    assert.string(manifest.uuid, 'manifest.uuid');
    assert.func(callback, 'callback');
    ...
    self.client.mkdirp(archDir, function (err) {
        ...
        var serialized = JSON.stringify(manifest, null, 2);
        var opts = {
            md5: crypto.createHash('md5').update(serialized).digest('base64'),
