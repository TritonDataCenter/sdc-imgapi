 ...
 * @param callback {Function} `function (err)`
 */
Storage.prototype._mvImageFile = function (image, from, to, callback) {};
...
LocalStorage.prototype.storeFileFromStream =
    ...
    function onFileWriteStream(err) {
        ...
        self._mvImageFile(image, tmpName, filename, function (mErr) {
            if (mErr) {
                callback(mErr);
                return;
            }
...
LocalStorage.prototype._mvImageFile = function (image, from, to, callback) {
    assert.object(image, 'image');
    assert.string(image.uuid, 'image.uuid');
...
MantaStorage.prototype._mvImageFile = function (image, from, to, callback) {
    assert.object(image, 'image');
    assert.string(image.uuid, 'image.uuid');
    assert.string(from, 'from');
    assert.string(to, 'from');
