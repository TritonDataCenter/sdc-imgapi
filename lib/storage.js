...
LocalStorage.prototype.storeFileFromStream =
    ...
    assert.object(opts.stream, 'opts.stream'); // should be paused
    // TRITON-2447 was an issue where the stream was not paused and caused
    // data corruption on disk. So this assertion is here to prevent
    // otherwise silent corruption on disk. It's better to blow an assert
    // and fail the pull than it is to say that the pull worked and have the
    // image file on disk be corrupted. If there are changes to docker.js in
    // the future that leave the stream unpaused (or other latent bugs that
    // we may discover), this will help us figure out why we're having this
    // problem.
    assert.ok(opts.stream.isPaused(), 'opts.stream is not paused');
