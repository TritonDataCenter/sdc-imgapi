...
LocalStorage.prototype.storeFileFromStream =
    function storeFileFromStream(image, stream, filename, callback) {
    this.createImageFileWriteStream(image, filename, function (sErr, output) {
        if (sErr) {
            callback(sErr);
            return;
        }
...
MantaStorage.prototype.setup = function setup(callback) {
    assert.func(callback, 'callback');
    // Assumption for now: it is writable for us.
    this.log.info('manta mkdir -p %s', this.dir);
    this.client.mkdirp(this.dir, callback);
};
...
MantaStorage.prototype.storeFileFromStream =
    function storeFileFromStream(image, stream, filename, callback) {
    var manta = this;
    manta.createImageFileWriteStream(image, filename, function (sErr, path) {
        if (sErr) {
            callback(sErr);
            return;
        }

        manta.client.put(path, stream, {}, callback);
    });
};
