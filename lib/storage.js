...
function MantaStorage(options) {
    ...
    assert.string(config.datacenterName, 'options.config.datacenterName');
    ...
    this.dir = format('/%s/stor/imgapi/%s', this.user, config.datacenterName);
...
MantaStorage.prototype.createExportDirectory =
function (prefix, callback) {
    assert.string(prefix, 'prefix');
    assert.func(callback, 'callback');

    this.client.mkdirp(prefix, function (err) {
        if (err) {
            return callback(err);
        }
        callback(null);
...
MantaStorage.prototype.exportManifestFromStream =
function (image, stream, string, prefix, callback) {
    var self = this;
    var ext = 'imgmanifest';
    var filename = format('%s-%s.%s', image.name, image.version, ext);
    var storPath = path.resolve(prefix, filename);
    var opts = {
    ...
    self.createExportDirectory(prefix, function (sErr) {
        if (sErr) {
            onFileWriteStream(sErr);
            return;
        }
        self.client.put(storPath, stream, opts, onFileWriteStream);
        stream.end(string);
    });
...
function (image, file, prefix, callback) {
    assert.object(image, 'image');
    assert.string(image.uuid, 'image.uuid');
    ...
    var ext = 'zfs' + exts[file.compression || 'none'];
    var filename = format('%s-%s.%s', image.name, image.version, ext);
    var fromPath = this._storPathFromImageUuid(image.uuid, 'file0');
    var toPath = path.resolve(prefix, filename);

    this.client.ln(fromPath, toPath, function (err) {
        if (err) {
            return callback(err);
        }
        return callback();
