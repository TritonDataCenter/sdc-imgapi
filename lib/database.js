...
UfdsDatabase.prototype.search = function (options, callback) {
    ...
    if (fields.length === 0) {
        ...
        for (var i = 0; i < fields.length; i++) {
            ...
            // When destroyed is passed as false then we add
            // (!(state=destroyed)) to the query
            if (field === 'destroyed' && value === false) {
                var destroyedFilter = new filters.EqualityFilter(
                    {attribute: 'state', value: 'destroyed'});
 ...
 * @param callback {Function} `function (err, items)`
 */
MorayDatabase.prototype.search = function (ldapFilter, callback) {};
 ...
 * @param callback {Function} `function (err)`
 */
MorayDatabase.prototype.modify = function (uuid, changes, callback) {};
