 ...
 * @param options.limit {Number} Return at most `limit` number of images.
 * @param options.marker {Number} Return images with a creation date
 *      greater or equal than the marker
...
LocalDatabase.prototype.search = function (options, callback) {
    ...
    var log = this.log;
    log.trace({ options: options }, 'LocalDatabase.options');

    // Normalize bools in the filter options for comparison below.
    var rawOptions = objCopy(filter);
    ...
    for (var m = 0; m < nManifests; m++) {
        ...
        if (options.limit && options.limit === m) {
            break;
        }
    }
 ...
 * @param options.limit {Number} Return at most `limit` number of images.
 * @param options.marker {Number} Return images with a creation date
 *      greater or equal than the marker. For a MorayDatabase, marker is the
 *      auto incremental _id column value of an image manifest object
 * @param limitOpts {Object} Optional limitOpts. Can pass limit and/or marker.
 * @param callback {Function} `function (err, items)`
 */
MorayDatabase.prototype.search = function (options, callback) {
    ...
    if (fields.length === 0) {
        ...
        if (options.marker) {
            ldapFilter.addFilter(new filters.GreaterThanEqualsFilter(
                { attribute: '_id', value: String(options.marker) }));
        }
        if (options.limit) {
    ...
    findOpts.sort = { attribute: '_id', order: 'ASC' };
