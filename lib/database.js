...
function UfdsDatabase(config, log) {
    ...
    assert.string(config.bindDN, 'config.bindDN');
    assert.string(config.bindPassword, 'config.bindPassword');
...
/**
 * Poll UFDS to get a bound client. This sets `this.client` as a side-effect.
 * It returns right away.
 */
UfdsDatabase.prototype._getClient = function _getClient() {
    var self = this;
    var attempts = 1;
    var timeout = null;

    var config = objCopy(self.config);
    ...
    function initUfdsRetry() {
        ...
        // Debugging IMGAPI-156/IMGAPI-161: log timeout errors from the
        // client.
        client.on('timeout', function () {
            self.log.warn('UFDS client timeout event');
        });
    }
...
UfdsDatabase.prototype.get = function (uuid, callback) {
    ...
    self.client.search(dn, {scope: 'base'}, function (err, items) {
        if (err) {
...
UfdsDatabase.prototype.search = function (options, callback) {
    ...
    self.client.search(base, opts, function (sErr, items) {
        if (sErr) {
...
UfdsDatabase.prototype.add = function (uuid, data, callback) {
    ...
    var dn = Image.dn(uuid);
    try {
        self.client.add(dn, data, function (addErr) {
            if (addErr) {
...
UfdsDatabase.prototype.modify = function (uuid, changes, callback) {
    ...
    var dn = Image.dn(uuid);
    self.client.modify(dn, changes, function (modErr) {
        if (modErr) {
...
UfdsDatabase.prototype.del = function (uuid, callback) {
    ...
    var dn = Image.dn(uuid);
    self.client.del(dn, function (delErr) {
        if (delErr) {
            if (delErr.restCode === 'ResourceNotFound') {
                callback(new errors.ResourceNotFoundError(
                    err, 'image ' + uuid + ' not found'));
            } else {
