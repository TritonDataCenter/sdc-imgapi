 ...
 * @param callback {Function} `function (err, items, _id)`
 */
 ...
 * @param limitOpts {Object} Optional limitOpts. Can pass limit and/or marker.
 * @param callback {Function} `function (err, items)`
 */
Database.prototype.search = function (ldapFilter, limitOpts, callback) {};
            ...
            function oneFile(filename, next) {
                ...
                function onFileStats(statErr, stats) {
                    ...
                    fs.readFile(filepath, 'utf8', function (err, content) {
                        ...
                        self.manifests.push({ value: raw, _id: mtime });
                        self.manifestFromUuid[uuid] = { value: raw, _id: mtime};
                        next();
                    });
                }
            },
 ...
 */
LocalDatabase.prototype._addManifest = function _addManifest(raw, _id) {
    var existing = this.manifestFromUuid[raw.uuid];
    if (existing === undefined) {
        this.manifests.push({ value: raw, _id: _id });
    } else {
        for (var i = 0; i < this.manifests.length; i++) {
            if (this.manifests[i].value.uuid === raw.uuid) {
                this.manifests[i] = { value: raw, _id: _id };
                break;
            }
        }
    }
    this.manifestFromUuid[raw.uuid] = { value: raw, _id: _id };
};
...
LocalDatabase.prototype._writeManifest = function (manifest, callback) {
    ...
    fs.writeFile(manifestPath, serialized, 'utf8', function (err) {
        ...
        fs.stat(manifestPath, onFileStats);
        ...
        function onFileStats(statErr, stats) {
            ...
            var mtime = stats.mtime.getTime();
            self._addManifest(manifest, mtime);
            callback();
        }
    });
};
...
LocalDatabase.prototype.get = function (uuid, callback) {
    ...
    callback(null, manifest.value, manifest._id);
};


LocalDatabase.prototype.search = function (options, limitOpts, callback) {
    assert.object(options, 'options');
    if (typeof (limitOpts) === 'function') {
        callback = limitOpts;
        limitOpts = undefined;
    }
    assert.func(callback, 'callback');
    ...
    for (var m = 0; m < nManifests; m++) {
        ...
        for (var f = 0; f < nFields; f++) {
            ...
            if (field === 'expires_at' && rawOptions[field] !== undefined) {
                if (manifest[field] > rawOptions[field]) {
                    match = false;
                }
                continue;
            } else if (type === '~str' && rawOptions[field][0] === '~') {
                ...
                var values = (typeof (rawOptions[field] === 'string') ?
                    [rawOptions[field]]: rawOptions[field]);
        ...
        // Lastly, check if a marker was passed to the query
        if (limitOpts && limitOpts.marker) {
            if (_id < limitOpts.marker) {
                match = false;
            }
        }
        if (match)
...
MorayDatabase.prototype.get = function (uuid, callback) {
    ...
    self.app.morayClient.getObject(IMAGES_BUCKET, uuid, function (err, obj) {
        if (err) {
            ...
            callback(null, obj.value, obj._id);
        }
    });
};

/**
 * Moray search. We can reuse Ufds' ldap query building
 *
 */
MorayDatabase.prototype.search = function (options, limitOpts, callback) {
    var self = this;
    assert.object(options, 'options');
    if (typeof (limitOpts) === 'function') {
        callback = limitOpts;
    ...
    assert.func(callback, 'callback');
    var log = self.log;
    log.trace({ options: options }, 'MorayDatabase.search');
    ...
    if (fields.length === 0) {
        ...
        if (limitOpts && limitOpts.marker) {
            ldapFilter.addFilter(new filters.GreaterThanEqualsFilter(
                { attribute: '_id', value: String(limitOpts.marker) }));
        }
        if (limitOpts && limitOpts.limit) {
            findOpts.limit = limitOpts.limit;
        }
    ...
    var filter = ldapFilter.toString();
    log.info({ filter: filter }, 'moray ldap search');
    var items = [];
    var req = self.app.morayClient.findObjects(IMAGES_BUCKET, filter, findOpts);
    ...
    req.on('record', function (object) {
        // Push _id to the items array so we can order images by creation date
        // inside apiListImages
        items.push({ value: object.value, _id: object._id });
    });
