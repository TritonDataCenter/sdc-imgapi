 ...
 * @param callback {Function} `function (err)`
 */
LocalDatabase.prototype._reload = function _reload(callback) {
    assert.func(callback, 'callback');
    var self = this;
    var log = this.log;
...
LocalDatabase.prototype.search = function search(options, callback) {
    ...
    // Normalize the filter options for comparison below.
...
LocalDatabase.prototype.del = function del(uuid, callback) {
    ...
    this.manifests.filter(function (i) { return i.uuid !== uuid; });
    var manifestPath = path.join(this.dir, uuid + '.raw'); rimraf(manifestPath, callback);
};
