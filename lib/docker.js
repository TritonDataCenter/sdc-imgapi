...
function dockerDownloadAndImportLayer(opts, callback) {
    ...
    function uncompressAndSha256Contents(arg, next) {
        // streampeek() inside of detectCompression leaves the stream unpaused
        // and this causes the storage driver to attach late and miss bytes
        // at the beginning of the file. So ensure the stream is paused
        // again.
        arg.stream.pause();
        ...
        // Is an additional explicit pause necessary here, given that we've
        // paused the stream at the beginning of this function? I don't know,
        // but I do know that explicitly pausing it after the pipe does not
        // hurt anything. If the stream isn't paused, then we know we get
        // corruption on disk, so I'm leaving it here.
...
function adminImportDockerImage(opts, callback) {
    ...
    vasync.pipeline({arg: context, funcs: [
        ...
        function handleV2ManifestList(ctx, next) {
            ...
            if (drc.manifestListTypes.indexOf(manifestV2.mediaType) === -1) {
                next();
                return;
            }
