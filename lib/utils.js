...
/**
 * Converts a key=value to a javascript literal (UFDS->Image)
 *
 * foo=bar
 * => { foo: 'bar' }
 */
...
function objectToKeyValue(obj) {
    ...
    Object.keys(obj).forEach(function (key) {
        // This doesn't affect image.tags because image.tag are only the indexed
        // values. It means that you can't search on big values
        value = (obj[key].toString().length <= MAX_VALUE_LENGTH ? obj[key]
                                                                : '');
