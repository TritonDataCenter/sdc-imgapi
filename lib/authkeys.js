...
function AuthKeysLoader(app) {
    ...
    this.loadDirs = [
        mod_path.join(constants.LOCAL_KEYS_DIR, 'local')
    ];
    if (app.config.manta) {
        this.mantaConfig = app.config.manta;
        this.mantaDir = mod_path.resolve(this.mantaConfig.rootDir, 'keys');
        this.mantaSyncDir = mod_path.join(constants.LOCAL_KEYS_DIR, 'manta');
