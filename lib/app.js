...
function App(config, log) {
    ...
    server.get({name: 'PublicDocs', path: /^\/docs\/(.+)?/},
        restify.serveStatic({
            default: 'public.html',
            directory: path.resolve(__dirname + '/../build/public-docs')
        }));
...
App.prototype.setup = function (callback) {
    ...
    if (self.config.ufds) {
        // Start creation of UFDS client: this sets `self.ufdsClient` at some
        // point in the future once successful connect and bind.
        self._getUfdsClient();
    }
 ...
 */
App.prototype._getUfdsClient = function _getUfdsClient() {
    var self = this;
    self.ufdsClient = null;
    ...
    ufdsConfig.cache = false;  // for now at least, no caching in the client
...
App.prototype.storFromImage = function storFromImage(image, preferredStor) {
    ...
    if (preferredStor === undefined) {
        ...
        var stor = this.storage[preferredStor];
        if (stor === undefined) stor = this.storage.local;
