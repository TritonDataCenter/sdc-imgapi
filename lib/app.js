...
App.prototype.setup = function setup(callback) {
    ...
    this.db.setup(function (dbErr) {
        ...
        self.storage = {};
        var names = Object.keys(self.config.storage);
        for (var i = 0; i < names.length; i++) {
            var name = names[i];
            var config = self.config.storage[name];
            var StorageClass = storage[name];
            log.info({name: name, config: config}, "create storage handler");
            try {
                self.storage[name] = new StorageClass({log: log, config: config});
            } catch (ctorErr) {
...
App.prototype.getStateSnapshot = function () {
    ...
    Object.keys(this._cacheFromScope).forEach(function (scope) {
        snapshot.cache[scope] = this._cacheFromScope[scope].dump();
    })
    return snapshot;
};
