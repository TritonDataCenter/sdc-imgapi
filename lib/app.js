...
function apiPing(req, res, next) {
    if (req.query.error !== undefined) {
        var errClass = ruhroh.errorClassFromString(mod_errors,
            req.query.error, false);
        var err;
        var errors;
        var message = req.query.message || "";
        if (req.query.errors) {
            errors = JSON.parse(req.query.errors); // no guard on this
        }
        if (req.query.cause !== undefined) {
            var causeClass = ruhroh.errorClassFromString(mod_errors,
                req.query.cause, true);
            var cause = new causeClass("cause-error-message")
            if (errors) {
                err = new errClass({message: message, cause: cause,
                    errors: errors});
            } else {
                err = new errClass(cause, message);
            }
        } else if (errors) {
            err = new errClass({message: message, errors: errors});
        } else {
            err = new errClass(message);
        }
        next(err);
    } else {
        var data = {
